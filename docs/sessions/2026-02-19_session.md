...
### Phase 8: Website Copy & Documentation Audit (commit 31f3b11)

Audited all customer-facing copy across `metricDetailData.ts`, `methodology/page.tsx`, and `status/page.tsx` for accuracy after adding new data sources.

**metricDetailData.ts changes (all 8 metrics):**
- Added Hacker News entries to collectionProgress/dataSources for 5 metrics (AI Psychosis, Subscription Overload, Wage Stagnation, Housing Despair, Layoff Watch)
- Added CFPB entries to collectionProgress/dataSources for 2 metrics (Healthcare, Housing Despair)
- Removed stale TikTok entries from 5 metrics where latest collection has 0 data (Wage Stagnation, Housing Despair, Airline Chaos, Dating App Despair, Layoff Watch)
- Removed all Google Trends references (4 metrics)
- Removed "App Store: 0 reviews collected (pending)" from AI Psychosis
- Removed "Industry Reports" from Subscription Overload collectionProgress (not a collected source)
- Updated methodology descriptions to list actual active platforms per metric

**methodology/page.tsx changes:**
- Sample size table: replaced TIKTOK column with OTHER (sums TikTok + HN + CFPB + Bluesky) with footnote explaining what OTHER includes
- Objectivity section: added FRED and CFPB to official sources
- Bias Mitigation: now mentions all 7 platforms with specific demographic bias notes (HN tech skew, CFPB complaint bias)
- Update Schedule: replaced stale "Google Trends, ACSI scores" with accurate Weekly (Automated) / Periodic (Manual) breakdown
- Limitations: updated platform demographic note

**update_metric_data.py:**
- Added collectionProgress regex updaters for Hacker News, CFPB, and Bluesky so future pipeline runs automatically keep counts current (previously only YouTube/Reddit/TikTok had updaters)
...
### Commit 5: `10ae567` - New data sources and pipeline integration
...
### Commit 6: `31f3b11` - Website copy and documentation audit
- `lib/metricDetailData.ts` (add HN/CFPB to collectionProgress and dataSources, remove stale TikTok/Google Trends/App Store entries, update methodology descriptions)
- `app/methodology/page.tsx` (sample size table now shows OTHER column, fix Update Schedule/Bias Mitigation/Limitations, add CFPB/FRED to Objectivity)
- `data-collection/update_metric_data.py` (add collectionProgress regex updaters for HN, CFPB, Bluesky)
...
9. **Removed Industry Reports from collectionProgress.** It was showing as "100/100 (100%)" but those aren't collected data points -- they're background research citations. The official sources (Consumer Reports, streaming pricing data) are still listed in dataSources. The official score (40% weight) still incorporates industry statistics.
10. **OTHER column in methodology table.** Instead of adding 5+ platform columns (YouTube, Reddit, TikTok, HN, CFPB, Bluesky), grouped non-YouTube/Reddit sources into a dynamic OTHER column with a footnote. Scales as new sources are added.
...
4. **Investigate Bluesky API 403.** The public search API (`app.bsky.feed.searchPosts`) returns 403 from CDN level. May need auth or alternative endpoint.
5. **Engagement unit normalization.** Reddit "score", YouTube "views", HN "points", and Bluesky "likes" are all treated equivalently in the log10 weighting, but measure different things.
6. **TikTok coverage regression.** Latest TikTok collection only captured 3 of 8 metrics (AI Psychosis, Healthcare, Subscription Overload). Previous collections had all 8. May be a YouTube API search issue. 5 metrics currently show no TikTok data.
